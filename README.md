...
В рамках приведения проекта в соответствие с требованиями ИБ были выполнены следующие работы на VPS:
...
7. **Шифрование БД:** Включено TLS-шифрование для подключений к PostgreSQL.
8. **Безопасное хранение секретов:** Файл с секретами `.env` убран из директории с кодом (`deploy/vps/.env`) и перенесен в защищенную системную директорию `/etc/converter/` (права 600).
9. **Внутреннее шифрование (TLS):** Настроен зашифрованный межконтейнерный обмен (amqps и https) для RabbitMQ и MinIO с использованием самоподписанных сертификатов.
